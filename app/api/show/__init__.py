...
def fillter(data,worker_id):
    report = {
        "隐私评估报告": {
            ...
            "报告摘要": {
                ...
                "评价标准": {
                    "隐私风险评估": "隐私风险评估是指在数据处理过程中，评估数据在不被授权访问或非法推理中可能暴露敏感信息的风险。具体是指：识别隐私泄露的潜在路径与攻击方式，并衡量数据暴露程度，以便采取相应的保护措施。",

                    "隐私保护性度量": "隐私保护性度量是指隐私保护算法执行后，隐私信息被还原的难易程度。具体是指：攻击者或第三方通过观测到的部分信息来推断隐私信息的能力。推断能力越强，说明安全性越差；反之，说明保护效果越好。",

                    "隐私合规性": "隐私合规性是指经过隐私保护处理后的数据是否满足相关法律法规、行业标准和组织内部规范的要求。具体是指：隐私保护措施是否能够支撑对数据最基本的合规性需求，包括数据处理原则、访问控制、安全机制等内容。",

                    "数据可用性": "数据可用性是指原始数据在经过隐私保护处理后所保留的信息是否仍可满足业务功能或分析需求。具体是指：隐私处理带来的数据偏差大小，偏差越小，则说明可用性越高；偏差越大，说明处理影响越大，可用性越低。",

                    "数据质量与特征": "数据质量与特征是指数据本身所具有的属性，以及其在应用中的完整性、准确性、一致性和稳定性。具体是指：评估隐私保护措施对数据的基础属性（如类型、分布、完整性）是否造成显著影响，进而影响其可分析性和可信度，从而反映整体质量水平。"
                }
            },
            "隐私风险度量":{
                "评估结果":{},
    ...
    if "可用性评估结果" in data:
        ...
        report["隐私评估报告"]["可用性评估"] = {
            ...
            "可用性综合得分": {
                "指标摘要": f"本次可用性评估综合得分为 {item['rank']['可用性综合结果']}，代表整体数据实用性水平。",
                "结论": (
                    "分数越高，说明数据在隐私保护下可用性越强。"
                    if item['rank']['可用性综合结果'] > 0.7 else
                    "分数偏低，说明保护措施影响了数据的实用性，建议结合实际需求权衡隐私与可用性。"
                )
            }
        }
    ...
    if "合规性评估结果" in data:
        ...
        desensitization_strategy = (
            "本次数据脱敏采用了 K-匿名性、L-多样性、T-紧密性和 (α,k)-匿名性等综合隐私保护技术，"
        ...
        if norm_score < 0.6:
            compliance_level = "较低"
            compliance_comment = (
                f"合规性检测得分为{norm_score:.3f}，处于较低水平。建议尽快优化数据脱敏及合规性处理，提升隐私保护强度，降低合规风险。"
            )
        elif norm_score < 0.8:
            compliance_level = "中等"
            compliance_comment = (
                f"合规性检测得分为{norm_score:.3f}，处于中等水平。建议持续优化相关处理，提高数据安全与合规性。"
            )
        elif norm_score < 0.95:
            compliance_level = "良好"
            compliance_comment = (
                f"合规性检测得分为{norm_score:.3f}，已达到良好水平，符合绝大多数合规性要求。建议保持当前策略并持续关注行业新标准。"
            )
        else:
            compliance_level = "优秀"
            compliance_comment = (
                f"合规性检测得分为{norm_score:.3f}，处于优秀水平，完全满足GDPR及《个人信息保护法》等主要法规的合规要求。"
            )
        report["隐私评估报告"]["数据合规性"] = {
            "脱敏策略合规性综述": {
                "技术描述": desensitization_strategy,
                "合规性检测": compliance_comment,
                "合规性等级": compliance_level
            },
            "K-Anonymity": {
                "指标摘要": f"数据集评估结果：{item['K-Anonymity']}（K={K_value}），即每个等价类中至少包含{K_value}条记录。",
                "定义": "K-匿名性要求每个等价类中至少有K条记录，降低单条记录被重识别风险。",
                "结果分析": (
                    f"当前K-匿名性为{K_value}，满足匿名化标准。所有等价类均包含不少于{K_value}条记录，"
            ...
            "L-Diversity": {
                "指标摘要": f"隐私属性{L_attr or sa_list}评估结果：{L_string}，满足L={L_value}-多样性。",
            ...
            "(α,k)-Anonymity": {
                ...
                "定义": "(α,k)-匿名性要求等价类中敏感属性的频率不超过α，同时等价类中记录不少于k条，从而抑制频率攻击。",
                "结果分析": (
                    f"当前属性{AK_attr or sa_list}在各等价类中敏感属性最大频率不超过α={AK_alpha}，"
                    f"且每组不少于k={AK_k}条，综合满足(α,k)-匿名性。"
                ),
                "改进建议": [
                    "可适当降低α、提升k，强化匿名性。",
                    "对频率分布进行动态监控，防止敏感值聚集。"
            ...
            "T-Closeness": {
                ...
                "定义": "T-紧密性要求等价类中敏感属性的分布与全局分布的距离不超过T，降低推断攻击风险。",
                "结果分析": (
        ...
        report["隐私评估报告"]["隐私保护性度量"] = {
            "安全性综述": {
                "报告摘要": (
                    ...
                    f"当前综合安全性评分为{security_score:.4f}，评级为【{security_level}】。"
                ),
                "综合结论": (
                    f"综合各项指标，数据集整体隐私保护处于{security_level}水平。"
                    "建议持续关注高风险敏感属性和等价组，动态调整脱敏方案。"
                )
            },
            "分布泄露": {
            ...
            "风险等级总结": {
                "当前等级": security_level,
                "得分": f"{security_score:.4f}",
                "结论": (
                    f"本数据集安全性综合评分为{security_score:.4f}，评级为{security_level}。"
                    "各项度量均已覆盖，评估表明数据集隐私保护能力良好，个别高风险点建议加强保护。"
                )
            }
        }
    # 返回有序的 JSON 响应
