...
# 执行评估指令
@celery.task(bind=True,base=MyTask)
def start_evaluate(self,SA):
    print("########################################################################")
    print("*******************************评估启动**********************************")
    worker_id = start_evaluate.request.hostname
    logger.info('json评估任务启动，任务配置：%s %s',worker_id,SA)
    time.sleep(30)

# @celery.task(bind=True,base=MyTask)
# def mysql2json(self,sqladd,username,password,)

@celery.task(bind=True,base=MyTask)
def reid_risk(uuid,src_url,to_url,QIDs,SA,ID):
    from .risk import reidentity_risk_assess
    ridrisk,sarisk=reidentity_risk_assess(uuid,src_url,to_url,QIDs,SA,ID,2,0.0001,0)
    print("风险评估结果：",ridrisk)
    print('统计推断攻击结果',sarisk)
...
# @celery.task(bind=True,base=MyTask)
# def Desensitization_character():
#         ##匿名集数据特征
#         handler3 = Desensitization_data_character(self.K_Anonymity,self.L_diversity,self.T_closeness,"",self.json_address,global_uuid,"","","","","")
#         handler3.runL(Series_quasi)   

# @celery.task(bind=True,base=MyTask)
# def Desensitization_quality():
#         ##匿名数据质量评估
#         handler4 = Desensitization_data_quality_evalution(self.K_Anonymity,self.L_diversity,self.T_closeness,"",self.json_address,global_uuid,"","","","","")
#         handler4.runL(Series_quasi)                          ##传递准标识符集合，以及准标识符对应的数量
