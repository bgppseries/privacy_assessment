...
import itertools
import numpy as np
from celery_task.func.membership import mfDerivs
import copy
...
def forwardHalfPass(ANFISObj, Xs):
    ...
    for pattern in range(len(Xs[:,0])):
        ...
        # 报错：修改
        if pattern == 0:
            wNormalized = layerTwo/(wSum[pattern] + EPS)
        else:
            wNormalized = np.vstack((wNormalized,layerTwo/(wSum[pattern] + EPS)  ))

        #prep for layer four (bit of a hack)
        layerThree = layerTwo/(wSum[pattern] + EPS)
