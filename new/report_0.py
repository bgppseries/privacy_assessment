...
# 配置 Redis 连接
redis_host = '10.10.55.24'  # 替换为你的 Redis 服务器地址
redis_port = 6379  # Redis 端口
redis_password = 'qwb'  # 如果有密码，填写密码

# 连接到 Redis 数据库
def connect_redis():
    return redis.StrictRedis(host=redis_host, port=redis_port,db=5, password=redis_password, decode_responses=True)
...
def fillter(data,worker_id):
    ...
    if "隐私保护性度量评估结果" in data:
            ...
            # 调整顺序，将 "评估任务ID" 放在第一个，"报告摘要" 放在第二个
...
## 首先对原始数据进行初步替换，
# 替换后的数据
replaced_data = replace_keys(raw_data)

# 输出替换结果
import json

### print()
json.dumps(replaced_data, ensure_ascii=False, indent=4)

worker_id="86d222f8-2ec7-421c-93d8-5e78ef54d5d5"

report=fillter(replaced_data,worker_id)
